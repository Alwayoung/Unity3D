...
public class FirstSceneControl : MonoBehaviour, ISceneControl, IUserAction {   
    ...
    private void Update()  
    {          
        if (actionManager.DNum == 0 && gameState == GameState.RUNNING)  
        {  
            gameState = GameState.ROUND_FINISH;  
              
        }  
  
        if (actionManager.DNum == 0 && gameState == GameState.ROUND_START)  
        {  
                curRound = (curRound + 1) % round;  
                NextRound();  
                actionManager.DNum = 10;  
    ...
    {  
        ...
        {  
            diskQueue.Enqueue(df.GetDisk(curRound));  
        }  
