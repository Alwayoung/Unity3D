...
public class CCMoveToAction : SSAction
{
	...
	public override void Start ()
	{
		...
		times = Vector3.Distance (pointA, pointB) / shotSpeed;
		this.gameObject.transform.position = pointA;
		speedv3 = new Vector3 ((pointB.x - pointA.x) / times,
			(pointB.y - pointA.y) / times - 0.5f * g * times, (pointB.z - pointA.z) / times);
		Gravity = Vector3.zero;
	}

	public static CCMoveToAction GetSSAction (float speedd)
	{
		CCMoveToAction action = ScriptableObject.CreateInstance<CCMoveToAction> ();
		action.speedd = action.shotSpeed = speedd;
		return action;
	}
